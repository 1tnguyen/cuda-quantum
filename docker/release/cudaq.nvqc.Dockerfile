...
RUN python3 -m pip install Flask fire
...
RUN echo 'python3 /nvqc_proxy.py run & if [[ "$NUM_GPUS" == "$(nvidia-smi --list-gpus | wc -l)" ]]; then while true; do mpiexec -np $(nvidia-smi --list-gpus | wc -l) cudaq-qpud --type nvcf --port 3031; done; else echo "Invalid Deployment: Number of GPUs does not match the hardware" && exit 1; fi' > launch.sh
