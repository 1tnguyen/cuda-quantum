...
typedef struct {
  ...
  int skip_tx_markers; // when non-zero, the host dispatcher will NOT write
                       // sentinel markers (0xEEEE) to tx_flags before graph
                       // launch.  Set this when an external GPU kernel (e.g.
                       // Hololink TX) polls the same tx_flags array; the
                       // sentinel would be misinterpreted as a valid address.
