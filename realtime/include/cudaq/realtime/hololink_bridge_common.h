...
struct BridgeConfig {
  ...
  cudaq_dispatch_path_t backend = CUDAQ_DISPATCH_PATH_DEVICE;
...
inline void parse_bridge_args(int argc, char *argv[], BridgeConfig &config) {
  for (int i = 1; i < argc; i++) {
    ...
    if (arg.find("--device=") == 0)
      ...
      config.backend = CUDAQ_DISPATCH_PATH_HOST;
  }
...
inline int bridge_run(BridgeConfig &config) {
  ...
  bool is_host_loop = (config.backend == CUDAQ_DISPATCH_PATH_HOST);
  bool use_forward = config.forward || (config.unified && !is_igpu);
  bool use_3kernel =
      is_host_loop || (!config.forward && !config.unified);
  ...
  if (!config.forward) {
    ...
    std::cout << "\n[4/5] Wiring dispatch kernel ("
              << (is_host_loop ? "host-loop" : config.unified ? "unified"
                                                              : "3-kernel")
              << ")..." << std::endl;
    ...
    *shutdown_flag = 0;
    ...
    if (is_host_loop) {
      ...
      if (cudaq_dispatcher_set_function_table(dispatcher, &table) !=
          CUDAQ_OK) {
