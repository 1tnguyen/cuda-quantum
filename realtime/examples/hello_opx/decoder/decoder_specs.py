#!/usr/bin/env python3
"""Decoder job catalog for the realtime OPNIC QUA examples."""
...
DEFAULT_TRT_ONNX_PATH = Path(
    os.environ.get(
        ...
        "/tmp/cudaqx-decoder-hostcall-build/surface_code_decoder.onnx",
    )
)
DEFAULT_TRT_GOLDEN_PATH = Path(
    os.environ.get(
        "CUDAQX_TRT_GOLDEN_PATH",
        Path(__file__).resolve().parent / "data" / "trt_surface_code_golden.txt",
    )
)
