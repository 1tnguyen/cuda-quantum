...
def bake_arrays(
    ...
    """Flatten the dataset into constant arrays baked into OPX data memory."""
    bit_len = len(shots[0])
    for idx, bits in enumerate(shots):
        if len(bits) != bit_len:
            raise RuntimeError(
                ...
                f"{bit_len}. Use an explicit --batch-size for ragged datasets."
            )
...
def build_program(
    ...
    arr_total = sum(n for _, n in arr_sizes)
    print("[QUA] Baked array data memory (int32, 4 B each):", file=sys.stderr)
    for name, count in arr_sizes:
        print(f"         {name:<12} {count:>8,} ints  "
              f"{_human_bytes(4 * count):>10}", file=sys.stderr)
    print(f"         {'TOTAL':<12} {arr_total:>8,} ints  "
          f"{_human_bytes(4 * arr_total):>10}", file=sys.stderr)

    with program() as prog:
        # Declare both streams so each QUA phase has the same signature as the
        # standing two-stream server. This phase only uses the data stream.
        declare_struct(DecoderControlRequestPacket)
        declare_struct(DecoderControlResponsePacket)
        ...
        if phase_done:
            # Mark this data phase complete. The host consumes function_id == 0
            # locally, recreates OPNIC contexts, and then syncs with the next
            # QUA program without unloading the shim or decoder bank.
            assign(data_req.magic[0], RPC_MAGIC_REQUEST)
            assign(data_req.function_id[0], 0)
