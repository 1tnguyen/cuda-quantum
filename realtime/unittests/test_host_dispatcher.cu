...
TEST_F(HostDispatcherLoopTest, BackpressureWhenAllBusy) {
  ...
  // With sweep_completed_workers, the host dispatcher auto-releases workers
  // when their graph streams complete.  Both slots will be processed
  // sequentially (single worker) without manual RestoreWorker calls.
