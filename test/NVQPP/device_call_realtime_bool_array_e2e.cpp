...
// compile one CUDA-Q app and exercise shared-memory, TCP external, and TCP
// auto-launch through the same co-linked service library.
...
int main(int argc, char **argv) {
  ...
  std::printf("device_call %s bool flat-array result = %d\n", label, boolValue);

  int intValue = intArrayKernel();
  std::printf("device_call %s i32 flat-array result = %d\n", label, intValue);
...
// TCP-EXTERNAL: device_call TCP external bool flat-array result = 19
// TCP-EXTERNAL: device_call TCP external i32 flat-array result = 42
// TCP-AUTO: device_call TCP auto bool flat-array result = 19
// TCP-AUTO: device_call TCP auto i32 flat-array result = 42
