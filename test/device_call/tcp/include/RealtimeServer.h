...
public:
  ...
  static const char *consumeValue(int &index, int argc, char **argv,
    ...
    return cudaq_internal::device_call::consumeValue(index, argc, argv,
                                                      current, option);
  }

  static bool parseUInt(const char *value, std::uint64_t maxValue,
                        std::uint64_t &out) {
    return cudaq_internal::device_call::parseUInt(value, maxValue, out);
  }
  ...
  static bool parseCommonOptions(int argc, char **argv, Options &options) {
    ...
    return cudaq_internal::device_call::parseCliOptions(
        argc, argv, commonOptions, options);
  }
  ...
  int run(int argc, char **argv,
    ...
    return run(argc, argv, bridgeInterface, options, Hooks{}, getService);
  }
  ...
  int run(int argc, char **argv,
    ...
    launchFn(
        ...
        functionEntries, functionCount, shutdownFlag, stats, options.numSlots,
        options.numBlocks, options.threadsPerBlock, stream);
...
private:
  ...
  bool initializeService() {
    if (getServiceFactory(&service) != 0 ||
        !service.get_function_count || !service.populate_table ||
        !service.get_device_dispatch_launch) {
