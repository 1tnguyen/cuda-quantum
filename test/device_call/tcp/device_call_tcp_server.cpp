...
constexpr const char DeviceCallServiceFactorySymbol[] =
    "cudaq_realtime_get_service";

using ServiceConfigureFn = int (*)(int, char **);

std::string sanitizeServiceName(const std::string &name) {
  std::string result;
...
std::string servicePostfixFromLibraryPath(const std::string &path) {
  ...
  if (so != std::string::npos)
    ...
    filename.erase(filename.size() - 6);
  else if (filename.ends_with(".dll"))
    filename.erase(filename.size() - 4);

  return sanitizeServiceName(filename);
}

std::string serviceFactorySymbol(const std::string &servicePostfix) {
  if (servicePostfix.empty())
    return DeviceCallServiceFactorySymbol;
  return std::string(DeviceCallServiceFactorySymbol) + "_" +
         sanitizeServiceName(servicePostfix);
}
...
loadAndInitializeService(int argc, char **argv, const Options &options) {
  ...
  const std::string servicePostfix =
      servicePostfixFromLibraryPath(options.serviceLibrary);
  if (!resolveDeviceCallServiceFactory(service->handle, servicePostfix)) {
    fail("service library does not export " +
         serviceFactorySymbol(servicePostfix) + " or " +
         serviceFactorySymbol({}));
    return nullptr;
  }
