...
std::int32_t runDeviceCallAbi(Fn &&fn) noexcept {
  ...
  } catch (const DeviceCallError &error) {
    return toAbiStatus(error.status());
  } catch (const std::invalid_argument &) {
    return toAbiStatus(DeviceCallStatus::InvalidArgument);
  } catch (const std::length_error &) {
    return toAbiStatus(DeviceCallStatus::ResponseTooLarge);
...
bool isGpuDispatchChannel(std::string_view name) {
  return name == GpuDispatchChannelName;
}

bool isHostDispatchChannel(std::string_view name) {
  return name == HostDispatchChannelName;
}

bool isBuiltinDispatchChannel(std::string_view name) {
...
public:
  ...
  DeviceCallRuntimeMode configuredMode() const {
    ...
    if (isHostDispatchChannel(config.channelName))
      return DeviceCallRuntimeMode::ManualHostDispatch;
  ...
  void initializeServiceForDevice(std::uint32_t deviceId,
    ...
    if (existingEndpoint && existingEndpoint->hasService())
      return;
    if (config.enabled && isHostDispatchChannel(config.channelName))
      throw DeviceCallError(DeviceCallStatus::NotInitialized,
                            "host device_call dispatch requires an explicit "
    ...
    args.channelName = GpuDispatchChannelName;
    args.arguments = config.arguments;
    args.channelConfig = makeChannelConfig(config);
    auto channel =
        createDeviceCallChannel(GpuDispatchChannelName, std::move(args));
    endpoint->setChannel(std::move(channel));
  ...
  void setHostFunctionTableForDevice(std::uint32_t deviceId,
    ...
    args.channelName = HostDispatchChannelName;
    args.arguments = config.arguments;
    args.channelConfig = makeChannelConfig(config);
    auto channel =
        createDeviceCallChannel(HostDispatchChannelName, std::move(args));
    endpoint->setChannel(std::move(channel));
    resetEndpointNoLock(deviceId);
    endpoints[deviceId] = std::move(endpoint);
    registerShutdownHandler();
...
void initializeSharedMemoryRuntime() {
  DeviceCallManager::instance().initializeService();
}
...
RuntimeModeActions getRuntimeModeActions(DeviceCallRuntimeMode mode) {
  ...
  case DeviceCallRuntimeMode::ManualHostDispatch:
...
void initializeDeviceCallRuntime(int argc, char **argv) {
  DeviceCallManager::instance().configure(argc, argv);
  getRuntimeModeActions(DeviceCallManager::instance().configuredMode())
      .initialize();
}
...
void setHostDeviceCallFunctionTableForDevice(std::uint32_t deviceId,
                                             cudaq_function_entry_t *entries,
                                             std::uint32_t count,
                                             void **mailbox) {
  DeviceCallManager::instance().setHostFunctionTableForDevice(deviceId, entries,
                                                              count, mailbox);
}
