...
class DeviceDispatchSharedMemoryChannel : public SharedMemoryChannelBase {
public:
  ~DeviceDispatchSharedMemoryChannel() override { stop(); }
  ...
  void acquireFrame(std::uint32_t functionId, std::uint64_t requestBytes,
    ...
    frame = {};
    if (!frameLengthsFitU32(requestBytes, responseCapacity))
...
CUDAQ_REGISTER_TYPE(DeviceCallChannel, DeviceDispatchSharedMemoryChannel,
                    device_dispatch)
