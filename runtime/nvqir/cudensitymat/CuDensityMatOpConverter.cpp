...
cudensitymatOperator_t
cudaq::dynamics::CuDensityMatOpConverter::constructLiouvillian(
    const superoperator &superOp, const std::vector<int64_t> &modeExtents,
    const std::unordered_map<std::string, std::complex<double>> &parameters) {
  LOG_API_TIME();
  cudensitymatOperator_t liouvillian;
  HANDLE_CUDM_ERROR(cudensitymatCreateOperator(
  ...
  for (const auto &[leftOpOpt, rightOpOpt] : superOp.get_terms()) {
    if (leftOpOpt.has_value()) {
      if (rightOpOpt.has_value()) {
        // Left and right multiplication
        for (const product_op<matrix_handler> &l_op_ : leftOpOpt->op) {
          for (const product_op<matrix_handler> &r_op_ : rightOpOpt->op) {
            auto leftCoeff = leftOpOpt->adjoint
                                 ? computeDagger(l_op_.get_coefficient())
                                 : l_op_.get_coefficient();
            auto rightCoeff = rightOpOpt->adjoint
                                  ? computeDagger(r_op_.get_coefficient())
                                  : r_op_.get_coefficient();
            scalar_operator coeff = leftCoeff * rightCoeff;
            {
              // L * rho * R
              std::vector<cudensitymatElementaryOperator_t> elemOps;
              std::vector<std::vector<std::size_t>> allDegrees;
              std::vector<std::vector<int>> all_action_dual_modalities;
              auto l_op = leftOpOpt->adjoint ? computeDagger(l_op_) : l_op_;
              auto r_op = rightOpOpt->adjoint ? computeDagger(r_op_) : r_op_;
              for (const auto &component : l_op) {
                if (const auto *elemOp =
                        dynamic_cast<const cudaq::matrix_handler *>(
                            &component)) {
                  auto cudmElemOp = createElementaryOperator(
                      *elemOp, parameters, modeExtents);
                  elemOps.emplace_back(cudmElemOp);
                  allDegrees.emplace_back(elemOp->degrees());
                  all_action_dual_modalities.emplace_back(
                      std::vector<int>(elemOp->degrees().size(), 0));
        ...
        // Left multiplication
        auto leftOp =
            leftOpOpt->adjoint ? computeDagger(leftOpOpt->op) : leftOpOpt->op;
        for (auto &[coeff, term] :
             convertToCudensitymat(leftOp, parameters, modeExtents)) {
          cudensitymatWrappedScalarCallback_t wrappedCallback =
              cudensitymatScalarCallbackNone;
          if (coeff.is_constant()) {
            const auto coeffVal = coeff.evaluate();
      ...
      if (rightOpOpt.has_value()) {
        ...
        auto rightOp = rightOpOpt->adjoint ? computeDagger(rightOpOpt->op)
                                           : rightOpOpt->op;
        for (auto &[coeff, term] :
             convertToCudensitymat(rightOp, parameters, modeExtents)) {
          cudensitymatWrappedScalarCallback_t wrappedCallback =
              cudensitymatScalarCallbackNone;
          if (coeff.is_constant()) {
            const auto coeffVal = coeff.evaluate();
