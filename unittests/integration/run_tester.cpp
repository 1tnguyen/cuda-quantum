...
CUDAQ_TEST(RunTester, checkQPE) {
  ...
  auto phases = cudaq::run(num_runs, qpe{}, 3, 1);
...
CUDAQ_TEST(RunTester, checkQPEAsync) {
#ifdef CUDAQ_BACKEND_TENSORNET
  constexpr int num_runs = 1;
#else
  constexpr int num_runs = 10;
#endif
  constexpr int qpu_id = 0;
  auto phases_future = cudaq::run_async(qpu_id, num_runs, qpe{}, 3, 1);
