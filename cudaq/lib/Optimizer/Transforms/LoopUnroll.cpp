  ...
  void runOnOperation() override {
    ...
    if (numLoops) {
      ...
      patterns.insert<UnrollCountedLoop>(ctx, threshold,
                                         ...
                                         progress, keepValueSemanticLoops);
    ...
    if (keepValueSemanticLoops) {
      // In the value-semantics opt-in mode, a loop that still indexes qubits by
      // a dynamic index (blocking wire conversion) but survived the unrolling
      // above (e.g. a non-constant trip count) is a dead end: it can be neither
      // kept in value semantics nor unrolled. Report each such loop.
      op->walk([&](cudaq::cc::LoopOp loop) {
        if (loop->hasAttr(cudaq::opt::DeadLoopAttr))
          return;
        if (loopBlocksWireConversion(loop)) {
          loop.emitOpError(
              "loop indexes qubits by the induction variable and cannot be "
              "unrolled (trip count is not a compile-time constant); it can be "
              "neither kept in value semantics nor unrolled");
          signalPassFailure();
        }
      });
